...
class OndmoneyNotificationServiceKafkaResourceIT {
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/ondmoney-notification-service-kafka/publish?message=value-produce").with(csrf()))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/ondmoney-notification-service-kafka/register"))
            .andExpect(status().isOk())
        ...
        for (int i = 0; i < 100; i++) {
            ...
            if (content.contains("data:value-consume")) {
                restMockMvc.perform(get("/api/ondmoney-notification-service-kafka/unregister"));
                return;
            }
        }
